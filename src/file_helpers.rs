...
pub fn file_read_vec_sparse(
    file: &mut File,
    field_size: usize,
    max: usize
) -> VecSparse {
    let n = read_u64(file, 4) as usize;
    let mut index: Vec<usize> = Vec::new();
    let mut value: Vec<BigUint> = Vec::new();
    for _i in 0..n {
        let idx = read_u64(file, 4) as usize;
        let val = read_big_uint(file, field_size);
    ...
    return VecSparse::with_data(n, max, index, value);
}
